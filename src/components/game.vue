<template>
  <section class="game">
    ...
    <el-dialog 
      ...
      <ol>
        <li>1. Make sure you have an EOS account.</li>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getByRollID(roll_id, counter, callback) {
        var roll_id_num = Number(roll_id);
        return api.getTableRows({
            json: true,
            code: 'fairdicegamd',
            table: 'bets',
            lower_bound: roll_id,
            // index_position: (roll_id_num + 1) + "",
            limit: 1,
            index_position: 2,
            key_type: "i64",
            scope: 'fairdicegamd'
          }).then(({ rows }) => {
            console.log("get activebets: ", JSON.stringify(rows));
            if (rows && rows.length == 1 && rows[0].roll_id == roll_id) {
                    callback(0, rows[0])
                } else {
                    if (counter > 20) {
                        callback(-1, 'timeout')
                    } else {
                        this.getByRollID(roll_id, counter + 1, callback)
                    }
                }
        });
      },
      ...
      getPool() {
        Promise.all([
          ...
          console.log("accountBalance:", accountBalance.rows[0].balance.slice(0, -4) )

          this.poolBalance = accountBalance.rows[0].balance.slice(0, -4) 
            - poolBalance.rows[0].locked;
      ...
      eos.transfer({
            ...
            memo: `${this.rollUnder}-${roll_id}` 
          }).then(() => {
            this.getEOS(); 
            // this.fetchResult(seed);
            var that = this
            this.getByRollID(roll_id, 0, function (code, data) {
              localStorage.setItem("current_id", data.id);
              var payout = data.payout_value / 10000
                if (payout === '0.0000') {
                  that.showDownAnimation = true; 
                  that.animationTxt = amount;
                } else {
                  that.showUpAnimation = true;
                  that.animationTxt = payout;
                }

                setTimeout(() => {
                  that.showDownAnimation = false; 
                  that.showUpAnimation = false;
                }, 3100);
